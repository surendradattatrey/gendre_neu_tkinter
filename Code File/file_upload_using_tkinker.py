# -*- coding: utf-8 -*-
"""
Created on Wed Jul  8 20:18:52 2020
...
def fileDialog():    
    global string
    filename = filedialog.askopenfilename(initialdir = "/",title="Select a file",filetype = (("All text Files","*.txt"),("All files","*.*")))
...
def my_click():       
    ...
    text_to_display = text.get(0.0,END)  
...
#Case single word in a line      
    ...
    read_file_button.destroy()
    next_button.destroy()
    
read_file_button  = Button(root, text = "Browse a File",command=fileDialog,font=('Helvetica',14,'bold'),bg="light blue")
read_file_button.grid(row=0,column=0,padx=800)
next_button = Button(root, text = "Next", command=my_click,font=('Helvetica',14,'bold'),bg="light blue") 
next_button.grid(row=1,column=0, padx=800)
